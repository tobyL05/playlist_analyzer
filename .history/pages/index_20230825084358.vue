...
const {code, state} = storeToRefs(storeAPIdetails());
...
onMounted(()=>{
	console.log("mounted");
	if(route.query != null){
		code.value = route.query.code?.toString() as string;
		state.value = route.query.state?.toString() as string;
		console.log("code " + code.to);
